...
import com.rich.music.service.impl.CollectServiceImpl;
...
//    添加收藏的歌曲
    ...
    public Object addCollection(HttpServletRequest req){
        ...
        collection.setType(new Byte(type));
        if (new Byte(type) == 0) {
        ...
        boolean res = collectService.addCollection(collection);
        if (res){
...
//        String song_list_id = req.getParameter("songListId").trim();
        ...
        Collection collection = new Collection();
        collection.setId(Integer.parseInt(id));
        collection.setUserId(Integer.parseInt(user_id));
        collection.setType(new Byte(type));
        collection.setSongId(Integer.parseInt(song_id));

        boolean res = collectService.updateCollectMsg(collection);
        if (res){
            jsonObject.put("code", 1);
